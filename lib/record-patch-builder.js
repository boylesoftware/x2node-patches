...
class AddRecordPatchOperation extends RecordPatchOperation {
	...
	apply(record, handlers) {
		...
		// determine if the operation needs to be applied
		let doit = false;
		const propDesc = this._pathPtr.propDesc;
		if (this._pathPtr.collectionElement) {
			doit = true;
		} else if (propDesc.isArray()) {
			if (propDesc.scalarValueType === 'object')
				doit = !equalObjectArrays(
					/*this._pathPtr.getValue(record), this._value*/);
			else
				doit = !equalSimpleArrays(
					this._pathPtr.getValue(record), this._value);
		} else if (propDesc.isMap()) {
			if (propDesc.scalarValueType === 'object')
				doit = !equalObjectMaps(
					/*this._pathPtr.getValue(record), this._value*/);
			else
				doit = !equalSimpleMaps(
					this._pathPtr.getValue(record), this._value);
		} else {
			if (propDesc.scalarValueType === 'object')
				doit = !equalObjects(
				...
				doit = (this._pathPtr.getValue(record) !== this._value);
		}
		...
		if (doit) {
			...
			if (handlers.onAdd)
				handlers.onAdd(this._pathPtr, this._value);
		}
...
class RemoveRecordPatchOperation extends RecordPatchOperation {
	...
	apply(record, handlers) {
		...
		if (this._pathPtr.collectionElement) {
			...
			doit = (curVal && (curVal.length > 0));
		} else if (propDesc.isMap()) {
			const curVal = this._pathPtr.getValue(record);
			doit = (curVal && (Object.keys(curVal).length > 0));
		} else {
			const curVal = this._pathPtr.getValue(record);
			doit = (curVal !== null);
		}
		...
		if (doit) {
			...
			if (handlers.onRemove)
				handlers.onRemove(this._pathPtr);
		}
...
class ReplaceRecordPatchOperation extends RecordPatchOperation {
	...
	apply(record, handlers) {
		...
		let doit = false;
		const propDesc = this._pathPtr.propDesc;
		if (!this._pathPtr.collectionElement && propDesc.isArray()) {
			if (propDesc.scalarValueType === 'object')
				doit = !equalObjectArrays(
					/*this._pathPtr.getValue(record), this._value*/);
			else
				doit = !equalSimpleArrays(
					this._pathPtr.getValue(record), this._value);
		} else if (!this._pathPtr.collectionElement && propDesc.isMap()) {
			if (propDesc.scalarValueType === 'object')
				doit = !equalObjectMaps(
					/*this._pathPtr.getValue(record), this._value*/);
			else
		...
		if (doit) {
			...
			if (handlers.onReplace)
				handlers.onReplace(this._pathPtr, this._value);
		}
