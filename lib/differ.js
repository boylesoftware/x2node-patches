 ...
 * @param {string} basePath JSON pointer path of the container (empty string if
 * top record).
		...
		if (propDesc.isArray()) {
			...
			if (valNew.length === 0) {
				...
				if (propDesc.scalarValueType === 'object')
					diffObjectArrays(
						propDesc, `${basePath}/${propName}`, valOld, valNew,
						patchSpec);
				else
					diffValueArrays(
						`${basePath}/${propName}`, valOld, valNew, patchSpec);
			}
 ...
 * @param {string} basePath JSON pointer path of the array property.
